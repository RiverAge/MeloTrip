...
class _AnimtedLyricsState extends State<_AnimtedLyrics> {
  ...
  _setPositionListner() async {
    ...
    _positionStream = player.positionStream.listen((position) {
      int currentLineIdx =
          _lyric.indexWhere((e) => (e.start ?? -1) > position.inMilliseconds);
      ...
      Scrollable.ensureVisible(gContext,
          alignment: 0.5, duration: const Duration(milliseconds: 650));
    });
  }
      ...
      return CurrentSongBuilder(builder: (context, current, songs, index, ref) {
        return AsyncValueBuilder(
            ...
            builder: (p0, lyrics, ref) {
              final lyric =
                  lyrics.subsonicResponse?.lyricsList?.structuredLyrics?.first;
