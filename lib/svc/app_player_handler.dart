...
class AppPlayer extends BaseAudioHandler {
  ...
  Future<void> _sequenceStateStreamListener(SequenceState? state) async {
    ...
    final url = await buildSubsonicUrl('/rest/getCoverArt?id=${song.id}',
        withHost: true);
  ...
  bool _distincSequenceStateStream(SequenceState? v1, SequenceState? v2) {
    ...
    if (prevSequence != null && currSequence != null) {
      ...
      return List.generate(prevSequence.length, (idx) => idx)
  ...
  List<SongEntity>? get playlist {
    ...
    final isLockCachingAudioSource =
        sources.every(((e) => e is LockCachingAudioSource));
    if (!isLockCachingAudioSource) return null;
    final songs = sources
        .map((e) => (e as LockCachingAudioSource).tag as SongEntity)
        .toList();
    return songs;
  }
  ...
  Future<void> setPlaylist({
    ...
    if (filterdSongs.length == _player.sequence?.length &&
        List.generate(filterdSongs.length, (idx) => idx).every((idx) =>
            filterdSongs[idx].id ==
            (_player.sequence?[idx].tag as SongEntity?)?.id)) {
      await skipToQueueItem(0);
    } else {
      final streamUrl = await buildSubsonicUrl('/rest/stream', withHost: true);

      final src = ConcatenatingAudioSource(
          useLazyPreparation: true,
          shuffleOrder: DefaultShuffleOrder(),
          children: filterdSongs
              .map((e) => LockCachingAudioSource(
  ...
  Future<void> addSongToPlaylist(SongEntity song,
      {bool? needPlay, bool? isNext}) async {
  ...
  Stream<List<SongEntity?>?> get sequenceStream => _player.sequenceStream
      .map((e) => e?.map((s2) => (s2.tag as SongEntity?)).toList());
  ...
  Future<void> customAction(String name, [Map<String, dynamic>? extras]) async {
    if (name == 'favorite') {
      ...
      await Http.get('/rest/${starred == "true" ? 'unstar' : 'star'}',
